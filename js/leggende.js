function get(scelta){
	...
	xmlhttp.onreadystatechange = function(){	/* onreadystatechange è il gestore di eventi*/
		if(this.readyState == 4 && this.status == 200 && scelta == 0){ /*quando è stata inviata e ricevuta la richiesta con successo (200="OK") con scelta = 0 allora viene chiamata la funzione leggende. readyState è una funzione di XMLHttpRequest che mantiene lo stato di XMLHttpRequest*/
			leggende(this);
		}
		if(this.readyState == 4 && this.status == 200 && scelta != 0){ /*quando è stata inviata e ricevuta la richiesta con successo (200="OK") con scelta != 0 allora viene chiamata la funzione filtro.*/
			filtro(this, scelta);
		}
	};
...
function leggende(xml){
	var i;
	var xmlDoc = cml.response.XML; /*responseXML restituisce l'oggetto richiesto*/
	var table="";	/* table è una stringa vuota che riceverà codice HTML e Javascript per costruire una vera e propria tabella */
	var x = xmlDoc.getEelementsByTagName("leggenda"); 
